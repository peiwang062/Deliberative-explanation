...
def main():
    ...
    difficulty_scores_te = 1 - np.max(all_class_dis_te, axis=1)
    ...
    np.save('./cub200/confidence_score_vgg16_layer42_recall.npy', recall)
    np.save('./cub200/confidence_score_vgg16_layer42_precision.npy', precision)
...
class ModelOutputs_cls():
    ...
    def __call__(self, x):
        target_activations, output = self.feature_extractor(x)
        output = output.view(output.size(0), -1)
        output = self.model._modules['module'].classifier(output)
